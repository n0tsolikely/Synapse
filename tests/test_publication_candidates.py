...
class PublicationCandidateTests(unittest.TestCase):
    ...
    def test_refresh_publication_candidates_writes_story_vision_and_codex_revisions(self) -> None:
        ...
        vision_candidate = resolve_publication_candidate(self.data_root, "vision")
        codex_body = Path(summary["current_codex_candidate_paths"][0]).read_text(encoding="utf-8")
        ...
        self.assertIn("## Intended Direction", story_text)
        self.assertIn("## Unresolved / Review", story_text)
        self.assertIn(str(canonical_project_story_path(self.data_root)), story_text)
        self.assertEqual(story_candidate["canonizer_schema_version"], 1)
        self.assertIn("canonizer_sections", story_candidate)
        self.assertTrue(story_candidate["canonizer_sections"]["truth_state_counts"])
        self.assertIn("## Source Refs", vision_text)
        self.assertIn("## Truths In Hand", vision_text)
        self.assertIn("## Intended Direction", vision_text)
        self.assertIn("## Unresolved / Review", vision_text)
        self.assertEqual(vision_candidate["canonizer_schema_version"], 1)
        self.assertIn("canonizer_sections", vision_candidate)
        self.assertTrue(vision_candidate["canonizer_sections"]["truth_state_counts"])
        self.assertIn("## Truths In Hand", codex_body)
        self.assertIn("## Intended Direction", codex_body)
        self.assertIn("## Unresolved / Review", codex_body)
        self.assertTrue(codex_manifest["source_refs"])
        self.assertTrue(codex_manifest["baseline_refs"])
        self.assertEqual(codex_manifest["canonizer_schema_version"], 1)
        self.assertIn("canonizer_sections", codex_manifest)
        self.assertTrue(codex_manifest["canonizer_sections"]["truth_state_counts"])
        self.assertTrue(summary["recent_story_candidate_details"][-1]["truth_state_counts"])
        self.assertTrue(summary["recent_vision_candidate_details"][-1]["truth_state_counts"])
        self.assertTrue(summary["recent_codex_candidate_details"][-1]["truth_state_counts"])
    ...
    def test_refresh_publication_candidates_is_noop_without_source_delta_change(self) -> None:
        ...
        first_story = resolve_publication_candidate(self.data_root, "story")
        first_vision = resolve_publication_candidate(self.data_root, "vision")
        first_codex = resolve_publication_candidate(self.data_root, "codex")
