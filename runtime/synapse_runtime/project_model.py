        ...
        normalized.append(
            {
                ...
                "requires_import_review": bool(item.get("requires_import_review", False)),
                "imported_confidence_band": str(item.get("imported_confidence_band") or "").strip() or None,
                "imported_source_count": int(item.get("imported_source_count") or 0),
    ...
    for item in authored_inputs:
        ...
        lines.extend(
            [
                ...
                f"- Review required: {bool(item.get('requires_import_review', False))}",
                f"- Imported confidence: {item.get('imported_confidence_band') or 'none'}",
                f"- Imported source count: {int(item.get('imported_source_count') or 0)}",
