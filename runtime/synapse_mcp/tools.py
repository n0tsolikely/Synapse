...
from synapse_mcp.runtime_bridge import (
    ...
    refresh_publication_candidates_tool,
    refresh_draftshot_tool,
    refresh_snapshot_candidates_tool,
...
def register_tools(mcp: FastMCP, state: ConnectionState) -> None:
    ...
    @mcp.tool(name="refresh_snapshot_candidates", structured_output=True)
    def _refresh_snapshot_candidates(
        context: ContextInput | None = None,
    ) -> dict[str, Any]:
        try:
            ctx, payload, event_info, status = refresh_snapshot_candidates_tool(
                state=state,
                context=context,
            )
            return _handle_mutation_result(ctx, payload, event_info, status)
    ...
    @mcp.tool(name="refresh_publication_candidates", structured_output=True)
    def _refresh_publication_candidates(
        context: ContextInput | None = None,
    ) -> dict[str, Any]:
        try:
            ctx, payload, event_info, status = refresh_publication_candidates_tool(
                state=state,
                context=context,
            )
            return _handle_mutation_result(ctx, payload, event_info, status)
    ...
    def _plan_quests(
        ...
        guild_orders_artifact: str | None = None,
