...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.codex_hook_json_stdin:
        try:
            ...
            return _emit_hook_warning("Synapse pre-tool capture failed because the Codex hook payload was not valid JSON.")
    ...
    if result.returncode != 0 or not args.close_turn:
        if result.returncode != 0 and hook_payload is not None:
            return _emit_hook_warning("Synapse pre-tool capture failed; continuity is degraded for this command.")
    ...
    if close_result.returncode != 0 and hook_payload is not None:
        return _emit_hook_warning("Synapse close-turn validation failed after pre-tool capture; continuity is degraded for this command.")
